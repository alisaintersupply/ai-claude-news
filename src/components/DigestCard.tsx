...
const typeConfig: Record<DigestType, { label: string; icon: React.ElementType; color: string }> = {
  daily: { label: 'สรุปประจำวัน', icon: FileText, color: 'text-pink-400' },
...
export const DigestCard = ({ digest }: DigestCardProps) => {
  ...
  return (
    <Link href={`/digest/${digest.id}`}>
      <article className="rounded-xl border border-pink-900/40 bg-slate-900/50 p-5 hover:border-pink-800/60 transition-colors group h-full">
        ...
        <h3 className="font-semibold text-slate-100 group-hover:text-pink-400 transition-colors mb-2 line-clamp-2">
